...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [customersRes, productsRes, ordersRes, paymentsRes] = await Promise.all([
          fetch(`${apiUrl}/api/customer/getCustomers`, { method: "GET", credentials: "include" }),
          fetch(`${apiUrl}/api/product/getproducts`, { method: "GET", credentials: "include" }),
          fetch(`${apiUrl}/api/order/getorders`, { method: "GET", credentials: "include" }),
          fetch(`${apiUrl}/api/payments/get`, { method: "GET", credentials: "include" })
        ]);
        ...
        if (customersRes.ok) setTotals(prev => ({ ...prev, customers: customersData.totalCustomers }));
        if (productsRes.ok) setTotals(prev => ({ ...prev, products: productsData.totalProducts }));
        if (ordersRes.ok) {
        ...
        if (paymentsRes.ok) {
          ...
          const totalSales = paymentsData.payments.reduce((acc, payment) => acc + payment.amount, 0);
          setTotals(prev => ({ ...prev, sales: totalSales }));
          if (paymentsData.payments.length < 9) setShowMorePayments(false);
        }

        if (isCustomer) {
          const ordersCustomerRes = await fetch(`${apiUrl}/api/order/getCustomerOrder/${currentUser._id}`, { method: "GET", credentials: "include" });
          const ordersCustomerData = await ordersCustomerRes.json();
          if (ordersCustomerRes.ok) setOrderCustomer(ordersCustomerData);
          
          const paymentsCustomerRes = await fetch(`${apiUrl}/api/payments/getPaymentCts/${currentUser._id}`, { method: "GET", credentials: "include" });
          const paymentsCustomerData = await paymentsCustomerRes.json();
          if (paymentsCustomerRes.ok) setPaymentsCustomer(paymentsCustomerData);
        }
      } catch (error) {
  ...
  const handleShowMorePayments = async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/payments/get?startIndex=${startIndex}`);
      const data = await res.json();
      if (res.ok) {
